...
const sourceBytes = new TextEncoder().encode(await Deno.readTextFile(file))
  .byteLength;
...
try {
  ...
  const gpupaperMedian = percentile(gpupaperMilliseconds, 0.5);
  const gpufuckMedian = percentile(gpufuckMilliseconds, 0.5);
  ...
  console.log(JSON.stringify(
    {
      ...
      runOrder: Array.from(
        ...
        .flat(),
      semanticEvidence: "deno task blot:verify",
      repositories: {
      ...
      environmentAtEnd,
      gpupaper: {
        p50Milliseconds: gpupaperMedian,
        p95Milliseconds: percentile(gpupaperMilliseconds, 0.95),
      ...
      gpufuck: {
        p50Milliseconds: gpufuckMedian,
        p95Milliseconds: percentile(gpufuckMilliseconds, 0.95),
        rawMilliseconds: gpufuckMilliseconds,
        wasmBytes: gpufuckWasm.byteLength,
      },
      gpupaperToGpufuckP50Ratio: gpupaperMedian / gpufuckMedian,
  ...
  for (const batchSize of [1, 4, 16, 64]) {
    ...
    measurements.push({
      ...
      queued: {
        ...
        p50Milliseconds: queuedMedian,
        p95Milliseconds: percentile(queuedMilliseconds, 0.95),
...
async function measureLiteralRebuilds(
  ...
  try {
    ...
    const measuredSamples = Math.min(samples, 5);
    ...
    return {
      ...
      gpupaper: {
        p50Milliseconds: percentile(gpupaperMilliseconds, 0.5),
        p95Milliseconds: percentile(gpupaperMilliseconds, 0.95),
        rawMilliseconds: gpupaperMilliseconds,
        invalidation: summarizeStage(gpupaperInvalidationMilliseconds),
      },
      gpufuck: {
        p50Milliseconds: percentile(gpufuckMilliseconds, 0.5),
        p95Milliseconds: percentile(gpufuckMilliseconds, 0.95),
        rawMilliseconds: gpufuckMilliseconds,
      },
    };
...
function requestedSampleCount(arguments_: readonly string[]): number {
  ...
  if (argument === undefined) return 15;
  const count = Number.parseInt(argument.slice("--samples=".length), 10);
  if (!Number.isSafeInteger(count) || count < 1) {
    throw new TypeError(`samples must be positive; received ${argument}`);
  }
  return count;
}
