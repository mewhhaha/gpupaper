...
#[unsafe(no_mangle)]
pub extern "C" fn run(seed: i32, generations: i32) -> i32 {
    let mut boards = [
        seed & 33_554_431,
        seed.wrapping_add(1) & 33_554_431,
        seed.wrapping_add(65_537) & 33_554_431,
        seed.wrapping_add(1_048_579) & 33_554_431,
    ];
    let mut remaining = generations;
    while remaining > 0 {
        let mut lane = 0;
        while lane < boards.len() {
            boards[lane] = generation(boards[lane]);
            lane += 1;
        }
        remaining -= 1;
    }
    boards[0]
        .wrapping_add(boards[1].wrapping_mul(3))
        .wrapping_add(boards[2].wrapping_mul(5))
        .wrapping_add(boards[3].wrapping_mul(7))
}
