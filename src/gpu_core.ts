import {
  commitTrustedCoreRewrites,
  type CoreRewriteProposal,
  proposeTrustedCoreRewrites,
} from "./core_rewrite.ts";
import {
  type FlatCore,
  FlatCoreKind,
...
export type GpuCoreResult =
  | {
    ...
    readonly inputProvenance: "validation";
...
type CompletedGpuCoreResult = Extract<
  GpuCoreResult,
  { readonly status: "completed" }
>;
...
fn constant_equals(value: ConstantDescriptor, expected_high: u32) -> bool {
  if (
    value.definition_kind !=
      ${FlatCoreKind.valueDefinition.operation}u
  ) {
    return false;
  }
  if (
    value.operation_kind != ${FlatCoreKind.operation.constant}u ||
    value.attribute_count != 1u
  ) {
    return false;
  }
  return
    value.attribute_kind == ${FlatCoreKind.attribute.number}u &&
...
fn propose_rewrites(@builtin(global_invocation_id) invocation: vec3<u32>) {
  ...
  if (
    ...
    candidate.result_type_kind != ${FlatCoreKind.type.scalar}u ||
    (
      candidate.result_scalar != ${FlatCoreKind.scalar.i32}u &&
      candidate.result_scalar != ${FlatCoreKind.scalar.i64}u
    )
  ) {
    return;
  }
  if (
    candidate.operator_kind !=
      ${FlatCoreKind.attribute.unsigned}u
  ) {
    return;
  }
  if (
    candidate.operator_value ==
      ${FlatCoreKind.binaryOperator.add}u
  ) {
    ...
    candidate.operator_value ==
      ${FlatCoreKind.binaryOperator.multiply}u
  ) {
...
type CoreGpuInput =
  ...
  | {
    ...
    readonly snapshot: TrustedFlatCore;
  }
  | {
...
const coreBatchQueue = createCompilerGpuBatchQueue(
  (inputs: readonly CoreGpuInput[]) =>
    inputs.length === 1
      ? Promise.all(
        inputs.map((input) => runCoreWithGpu(input, "latency")),
      )
      : runCoreGpuBatch(inputs),
);

export async function runCoreGpuPass(
  snapshot: FlatCore,
  options: {
    readonly scheduling?: CompilerGpuSchedulingPolicy;
  } = {},
): Promise<GpuCoreResult> {
  return await enqueueCoreGpuPass(
    { kind: "raw", package: snapshot },
    options.scheduling ?? "latency",
  );
}

export async function runTrustedCoreGpuPass(
  snapshot: TrustedFlatCore,
  options: {
    readonly scheduling?: CompilerGpuSchedulingPolicy;
  } = {},
): Promise<GpuCoreResult> {
...
async function enqueueCoreGpuPass(
  input: CoreGpuInput,
...
async function runCoreGpuBatch(
  inputs: readonly CoreGpuInput[],
): Promise<readonly GpuCoreResult[]> {
  try {
    return await runPackedCoreGpuBatch(inputs);
  } catch (error) {
    if (error instanceof CompilerGpuCapacityError && inputs.length > 1) {
      const split = Math.ceil(inputs.length / 2);
      const [left, right] = await Promise.all([
        runCoreGpuBatch(inputs.slice(0, split)),
        runCoreGpuBatch(inputs.slice(split)),
      ]);
      return [...left, ...right];
    }
    throw error;
...
async function runPackedCoreGpuBatch(
  inputs: readonly CoreGpuInput[],
): Promise<readonly GpuCoreResult[]> {
  ...
  if (rewriteJobs.length !== jobs.length) {
    const rewriteResults = rewriteJobs.length === 0
      ...
      ? [
        await runCoreWithGpu(
          preparedCoreGpuInput(rewriteJobs[0]),
          "latency",
        ),
      ]
      : await runCoreGpuBatch(
    ...
    return jobs.map((job, index) => {
      if (job.status === "rewrite") {
        ...
        if (result === undefined) {
          throw new Error(
            `Core batch omitted rewrite result for job ${index}`,
          );
        }
        return result;
      }
      if (job.status !== "identity") {
        throw new Error(
          `Core batch identity job ${index} has status ${job.status}`,
        );
      }
  ...
  try {
    ...
    return gpuJobs.map((job, index) => {
      ...
      const committed = commitTrustedCoreRewrites(
        job.snapshot,
        proposals,
      );
      return {
...
function prepareCoreJob(input: CoreGpuInput): PreparedCoreJob {
  ...
  const rewriteCandidateOperationIds = proposeTrustedCoreRewrites(
...
function validateCoreSnapshotForGpu(
  ...
  try {
    return {
      status: "valid",
      snapshot: validateFlatCore(snapshot),
    };
  } catch (error) {
    return {
      status: "invalid",
...
function coreRewriteCandidateDescriptors(
  ...
  const writeConstant = (
    ...
    if (
      definitionKind !== FlatCoreKind.valueDefinition.operation
    ) return;
...
function gpuRewriteProposals(
  ...
  return Array.from(rules.entries()).flatMap(([candidateId, ruleId]) => {
    ...
    if (rule === undefined) {
      throw new TypeError(
        `GPU proposed unknown Core rewrite rule ${ruleId} at operation ${operationId}`,
      );
    }
...
async function requestGpuCoreContext(): Promise<GpuCoreContext> {
  ...
  const pending = (async (): Promise<GpuCoreContext> => {
    ...
    try {
      ...
      return {
        status: "unavailable",
        reason: `WebGPU Core initialization failed: ${
