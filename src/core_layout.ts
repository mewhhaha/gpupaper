import type { CoreModule, CoreTypeId } from "./core.ts";
...
export type CoreLayoutPlan = {
  readonly layouts: readonly CoreLayout[];
...
export type CoreBufferOwnership = "owned" | "frozen";

export type CoreBufferRepresentation = {
 ...
 * at that boundary without changing core's semantic buffer kinds.
 */
export function coreBufferRepresentation(
  _ownership: CoreBufferOwnership,
): CoreBufferRepresentation {
  return {
    kind: "handle",
    size: handleSize,
...
export function planCoreLayouts(
  module: CoreModule,
): CoreLayoutPlan {
  const layouts: CoreLayout[] = [];
  ...
  const intern = (layout: CoreLayout): LayoutId => {
  ...
  const layoutOf = (type: CoreTypeId): LayoutId => {
    ...
    if (visiting.has(type)) {
      throw new Error(`non-recursive Core type ${type} forms a cycle`);
    }
    visiting.add(type);
    const entry = module.types[type];
    if (entry === undefined) {
      throw new RangeError(
        `Core type ${type} is outside table length ${module.types.length}`,
      );
    }
  ...
  return {
    ...
    typeLayouts: typeLayouts.map((id, index) => {
      if (id === undefined) {
        throw new Error(`Core type ${index} received no layout`);
      }
      return id;
    }),
  };
...
function computeLayout(
  ...
  switch (entry.kind) {
    ...
    case "buffer": {
      ...
      const representation = coreBufferRepresentation("owned");
      return {
        kind: "handle",
