...
export type TrustedFlatCore = {
  ...
  readonly provenance: "validation";
  readonly [trustedFlatCore]: true;
};
...
export function flattenCore(
  module: CoreModule,
): FlatCore {
  validateCore(module);
  const layouts = planCoreLayouts(module);
  ...
  const sourceId = (span: CoreSourceSpan): number => {
  ...
  return {
    ...
    functionSourceSymbolIds: new Uint32Array(
      module.functions.map((function_) =>
        function_.sourceIdentity === undefined
          ? absent
          : unsignedWord(function_.sourceIdentity, "source symbol ID")
      ),
    ),
...
export function flattenValidatedCore(
  module: CoreModule,
): TrustedFlatCore {
  validateCore(module);
  return {
    package: flattenCore(module),
    provenance: "validation",
    [trustedFlatCore]: true,
  };
}

export function inflateFlatCore(
  package_: FlatCore,
): CoreModule {
...
export function validateFlatCore(
  package_: FlatCore,
): TrustedFlatCore {
  inflateFlatCore(package_);
  return {
    package: package_,
    provenance: "validation",
    [trustedFlatCore]: true,
...
type ValidatedTables = {
  ...
  readonly sourceLocations: readonly CoreSourceSpan[];
};

function validateFlatStructure(
  package_: FlatCore,
): ValidatedTables {
  if (package_.schemaVersion !== flatCoreSchemaVersion) {
    throw new TypeError(
      `flat Core schema version must be ${flatCoreSchemaVersion}; received ${package_.schemaVersion}`,
    );
  }
  equalLengths(
    "string",
    ...
    (fileId, index) => {
      ...
      if (start > end) {
        throw new RangeError(
          `flat Core source location ${index} starts at ${start} after end ${end}`,
        );
      }
  ...
  if (package_.typeLayoutIds.length !== package_.typeKinds.length) {
    throw new TypeError(
      `flat Core type layout columns must have equal lengths; received ${package_.typeLayoutIds.length} and ${package_.typeKinds.length}`,
    );
  }
  ...
  for (const [typeId, kindId] of package_.typeKinds.entries()) {
    ...
    if (
      ...
      throw new RangeError(
        `flat Core type ${typeId} has invalid ${kind} auxiliary ${auxiliary}`,
      );
    }
    const expectedCount = kind === "product" || kind === "sum" ? count : 0;
    if (count !== expectedCount) {
      throw new TypeError(
        `flat Core ${kind} type ${typeId} cannot have ${count} payloads`,
      );
    }
  }
  ...
  for (
    ...
    if (entry < blockStart || entry >= blockStart + blockLength) {
      throw new RangeError(
        `flat Core function ${functionId} entry block ${entry} is outside its range ${blockStart}..${
    ...
    for (
      ...
      if (
        ...
        throw new TypeError(
          `flat Core block ${blockId} disagrees with function ${functionId} source order`,
        );
      }
    }
  }
  if (package_.blockTerminatorIds.length !== package_.terminatorKinds.length) {
    throw new TypeError(
      `flat Core requires one terminator per block; received ${package_.blockTerminatorIds.length} blocks and ${package_.terminatorKinds.length} terminators`,
    );
  }
  for (const [blockId, terminatorId] of package_.blockTerminatorIds.entries()) {
    if (
      ...
      throw new TypeError(
        `flat Core block ${blockId} does not own terminator ${terminatorId}`,
      );
    }
  }
  ...
  for (let valueId = 0; valueId < package_.valueLocalIds.length; valueId += 1) {
    ...
    if (valueByLocal.has(key)) {
      throw new TypeError(
        `flat Core function ${functionId} repeats local value ${localId}`,
      );
    }
    ...
    if (definitionKind === parameterDefinition) {
      ...
      if (package_.blockParameterValueIds[definitionId] !== valueId) {
        throw new TypeError(
          `flat Core value ${valueId} parameter definition ${definitionId} points elsewhere`,
        );
      }
      ...
      if (package_.operationResultValueIds[definitionId] !== valueId) {
        throw new TypeError(
          `flat Core value ${valueId} operation definition ${definitionId} points elsewhere`,
        );
      }
    } else {
      throw new TypeError(
        `flat Core value ${valueId} has unknown definition kind ${definitionKind}`,
      );
    }
  }
  ...
  for (
    ...
    for (
      ...
      if (package_.operationBlockIds[operationId] !== blockId) {
        throw new TypeError(
          `flat Core operation ${operationId} belongs to block ${
    ...
    for (let edgeId = edgeStart; edgeId < edgeStart + edgeCount; edgeId += 1) {
      ...
      if (package_.blockFunctionIds[target] !== functionId) {
        throw new TypeError(
          `flat Core edge ${edgeId} crosses function ${functionId} to ${
  ...
  for (const [attributeId, kindId] of package_.attributeKinds.entries()) {
    if (
      ...
      throw new RangeError(
        `flat Core attribute ${attributeId} uses string ID ${
      ...
      switch (typeKinds[kindId]) {
        ...
        case "store":
          if (payloads.length !== 1) {
            throw new TypeError(
              `flat Core store type ${typeId} has ${payloads.length} element types; expected one`,
            );
          }
  ...
  const functions = Array.from(
    ...
    (nameId, functionId): CoreFunction => {
      ...
      const entryBlock = package_.functionEntryBlockIds[functionId] -
        blockStart;
      const sourceIdentity = package_.functionSourceSymbolIds[functionId];
      return {
        id: functionId as CoreFunctionId,
        name: tables.strings[nameId],
        sourceIdentity: sourceIdentity === absent ? undefined : sourceIdentity,
  ...
  switch (kind) {
    ...
    case "scalar.binary":
      ...
      if (
        ...
        throw new TypeError(
          `flat Core scalar.binary has unknown operator ${attributes[0]}`,
        );
      }
      return {
        ...base,
    ...
    case "store.grow":
      ...
      if (attributes[0] !== 0 && attributes[0] !== 1) {
        throw new TypeError(
          `flat Core ${kind} has unknown update mode ${attributes[0]}`,
        );
      }
      return {
        ...base,
...
function inflateTerminator(
  ...
  if (kind === "branch") {
    if (edges.length !== 1) {
      throw new TypeError(
        `flat Core branch terminator ${terminatorId} has ${edges.length} edges`,
      );
    }
  ...
  if (kind === "conditional_branch") {
    if (edges.length !== 2) {
      throw new TypeError(
        `flat Core conditional terminator ${terminatorId} has ${edges.length} edges`,
      );
    }
    const condition = package_.terminatorConditionValueIds[terminatorId];
    if (condition === absent) {
      throw new TypeError(
        `flat Core conditional terminator ${terminatorId} has no condition`,
      );
    }
    return {
      kind,
  ...
  if (kind === "return") {
    if (edges.length !== 0) {
      throw new TypeError(
        `flat Core return terminator ${terminatorId} has ${edges.length} edges`,
      );
    }
  ...
  if (edges.length !== 0) {
    throw new TypeError(
      `flat Core trap terminator ${terminatorId} has ${edges.length} edges`,
    );
  }
  return { kind, span };
}
...
function appendConstantAttribute(
  value: Extract<CoreOperation, { readonly kind: "constant" }>["value"],
  pushAttribute: (
    kind: AttributeKind,
    lowWord: number,
    highWord?: number,
...
function decodeAttribute(
  ...
  switch (attributeKinds[package_.attributeKinds[attributeId]]) {
    ...
    case "boolean":
      if (low > 1) {
        throw new TypeError(
          `flat Core boolean attribute ${attributeId} has word ${low}`,
        );
      }
      return low === 1;
    case "string":
...
function collectSourceLocations(
  module: CoreModule,
): readonly CoreSourceSpan[] {
  const locations: CoreSourceSpan[] = [];
  const keys = new Set<string>();
  const add = (span: CoreSourceSpan): void => {
...
function sourceLocationKey(span: CoreSourceSpan): string {
...
function typeAuxiliary(type: CoreType): number {
  if (type.kind === "scalar") {
...
function validateLayouts(
  ...
  const expectedPlan = planCoreLayouts(module);
...
function requireAttributeCount(
  ...
  if (attributes.length !== expected) {
    throw new TypeError(
      `flat Core ${operation} has ${attributes.length} attributes; expected ${expected}`,
    );
  }
}
...
function requireValueFunction(
  ...
  if (package_.valueFunctionIds[valueId] !== functionId) {
    throw new TypeError(
      `flat Core ${subject} uses value ${valueId} from function ${
...
function decodeStrings(package_: FlatCore): readonly string[] {
  contiguousRanges(
    "string",
    package_.stringStarts,
    package_.stringLengths,
  ...
  const strings = Array.from(package_.stringStarts, (start, index) => {
    ...
    try {
      ...
      throw new TypeError(
        `flat Core string ${index} is not valid UTF-8`,
        { cause },
      );
    }
  });
  for (let index = 1; index < strings.length; index += 1) {
    if (compareStrings(strings[index - 1], strings[index]) >= 0) {
      throw new TypeError(
        `flat Core strings must be strictly ordered at ${index}`,
      );
    }
  }
  return strings;
...
function equalLengths(
  ...
  throw new TypeError(
    `flat Core ${subject} columns must have equal lengths; received ${
      lengths.join(", ")
    }`,
  );
}
...
function contiguousRanges(
  ...
  for (let index = 0; index < starts.length; index += 1) {
    ...
    if (start !== expected) {
      throw new RangeError(
        `flat Core ${subject} ${index} starts at ${start}; expected contiguous start ${expected}`,
      );
    }
    if (count > total - start) {
      throw new RangeError(
        `flat Core ${subject} ${index} range ${start}..${
          start + count
  ...
  if (expected !== total) {
    throw new RangeError(
      `flat Core ${subject} ranges cover ${expected}; column has ${total}`,
    );
  }
}
...
function requireIndex(index: number, length: number, subject: string): void {
  if (!Number.isSafeInteger(index) || index < 0 || index >= length) {
    throw new RangeError(
      `flat Core ${subject} ${index} is outside table length ${length}`,
    );
  }
}
...
function requiredKindId<T extends string>(
  ...
  if (id >= 0) return id;
  throw new TypeError(`unknown Core ${subject} kind ${kind}`);
}
...
function requiredStringId(
  ...
  if (id !== undefined) return id;
  throw new Error(`flat Core string was not interned: ${value}`);
}
...
function requireEqualWords(
  ...
  throw new TypeError(`flat Core ${subject} disagrees with Core`);
}
