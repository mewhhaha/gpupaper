...
The 2026-08-01 branch benchmark compares byte-identical semantic modules with
and without the standardized `metadata.code.branch_hint` custom section. The
condition is true for 999 of every 1,000 calls. Each fresh Deno process takes
...
| Process | hinted/unhinted module ms | hinted/unhinted ns per call | runtime change |
| ------: | ------------------------: | --------------------------: | -------------: |
|       1 |           0.00247/0.00226 |                 7.527/7.791 |         -3.38% |
|       2 |           0.00146/0.00135 |                 5.611/5.635 |         -0.42% |
|       3 |           0.00223/0.00204 |               10.105/10.729 |         -5.82% |
|       4 |           0.00271/0.00254 |                 5.206/5.369 |         -3.03% |
|       5 |           0.00145/0.00133 |                 5.541/5.628 |         -1.55% |
|       6 |           0.00256/0.00237 |                 9.865/9.623 |         +2.52% |

The artifact grows from 50 to 84 bytes, matching the 34-byte minimum custom
section cost. Five processes improve and one regresses. The paired median
runtime difference is -0.125 ns/call; hinted module construction is slower in
all six by a median 0.00018 ms. Excluding transport cost, those medians imply an
approximately 1,440-call amortization point. The process-level reversal is
retained: correct likelihood is not a guarantee that a particular run becomes
faster. This microbenchmark establishes useful V8 consumption for the selected
branch shape, not the speedup of an entire Ducklang application.

The retained static policy changes only the frozen Codex artifact, from the
historical 226,134-byte measurement to a current deterministic size of 226,211
bytes. The 77-byte delta is metadata; no current whole-Codex runtime comparison
has been recorded, so the historical timing tables below retain their original
artifact sizes.
...
template, subject to measuring argument-dependent reductions; temporary identity
...
reuse ceiling, not a dependency proof; the counters were removed before the next
subtree-dependency measurement.
...
epoch was rejected. Fifteen direct Codex baseline/cache/baseline rewrite samples
measured median/MAD 70.379/1.648, 76.230/2.688, and 71.077/3.744 ms. Dynamic
lookup at every entry costs more than same-context identity reuse; no cache code
remains.
...
targets produce none. Codex needs each hit to repay at least 26.73 lookups
before constant costs. It does not, matching the measured regression. Counters
...
Block-step lookup and removal perform only 6,258 Codex comparisons, 6.37% of the
remaining 98,211 capture-argument visits. Other targets perform at most 1,360.
The counter was removed; indexed mutable step lists are rejected for the current
corpus.
...
byte-identical, engine-valid sizes of 24,460, 226,134, 3,911, 26,106, 2,520, and
3,864 bytes. Release timings are recorded in `PAPER.md` as boundary observations
rather than benchmark distributions.
...
The historical gpupaper Blot path consumed owned GPU syntax for the closed `I64`
let/return fragment and emitted its Wasm on the GPU. One cold 32-byte compile
measured 531.08 ms runtime creation, 375.90 ms plan compilation, 134.70 ms owned
syntax, 2.73 ms host payload lowering and Wasm planning, 317.93 ms GPU emission
through a separately acquired runtime, and 3.90 ms engine validation. It emitted
43 bytes and returned `42n`. This single profile is diagnostic rather than a
distribution: setup, not the linear host lowering, was the measured small-input
target. The production path now consumes resident syntax and `benchmark:syntax`
has separate direct-ordinal and segmented binding-ordinal columns. It also
measures 32, 512, and 2,048-binding predecessor families with exactly
\(B(B+1)/2\) name candidates. Fresh RTX measurements are required before
replacing this baseline. A shared syntax/emission runtime remains unimplemented.

The 2026-08-01 derivation audit corrected two benchmark labels. Blot's binding
counts are `[1, ..., 1, 0]`, so the selected strategy is justified by a
direct-ordinal theorem, not count uniformity. The segmented strategy scans
binding counts only; it is not a general variable-cardinality payload emitter.
The direct cold path now compiles only its classifier and emitter instead of
also compiling the unselected segmented shaders.

The scan operates on conservative capacity
`C = min(nodeCapacity,
max(1, floor(sourceLength / 8)))`, not the exact resident
declaration count. For `q = ceil(log2(C + 1))`, exact conditional addition work
is `q(C + 1) - (2^q - 1)`. The benchmark now reports that value, its `q(C + 1)`
upper bound, scheduled invocations, and temporary bytes:

| bindings | capacity C | padded lanes |  q | exact additions | reported upper bound |
| -------: | ---------: | -----------: | -: | --------------: | -------------------: |
|       32 |         75 |           42 |  7 |             405 |                  532 |
|      512 |      1,318 |          805 | 11 |          12,462 |               14,509 |
|    8,192 |     23,276 |       15,083 | 15 |         316,388 |              349,155 |
|   32,768 |     99,625 |       66,856 | 17 |       1,562,571 |            1,693,642 |

Skipped trivia can increase all four capacity-derived costs without adding a
declaration. This is the adversarial case for a future resident-count indirect
dispatch or arena; no speedup is claimed until required-hardware stage timings
separate padded classification, scan, and name resolution.
